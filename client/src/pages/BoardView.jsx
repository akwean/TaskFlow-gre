...
import CardItem, { CardDragClone } from "../components/CardItem";
...
const BoardView = () => {
    ...
    const customCollisionDetection = useCallback((args) => {
        ...
        if (Math.random() < 0.01) {
            // Reduced from 5% to 1% to avoid spam
    ...
    useEffect(() => {
        const onOnline = () => {
            setOnlineState(true);
            add("Reconnected", "success");
        };
        const onOffline = () => {
            setOnlineState(false);
            add("You are offline. Changes will retry.", "error");
        };
    ...
    useEffect(() => {
        ...
        const handleListCreated = ({ list }) => {
            setLists((prev) =>
                prev.find((l) => l._id === list._id) ? prev : [...prev, list],
            );
        ...
        const handleCardUpdated = ({ card, oldListId, newListId }) => {
            ...
            setCards((prev) => {
                ...
                if (fromId && fromId !== toId && next[fromId]) {
                    next[fromId] = next[fromId].filter(
                        (c) => c._id !== card._id,
                    );
                }
        ...
        onSocket("list:presence", ({ listId, count }) => {
            setListPresence((prev) => ({
                ...prev,
                [listId]: Math.max(0, count || 0),
            }));
        });
    ...
    const onlineUsers = useMemo(() => {
        ...
        if (user?._id) {
            ...
            list = list.sort((a, b) =>
                a._id === me ? -1 : b._id === me ? 1 : 0,
            );
        }
        return list;
    }, [board, onlineUserIds, user]);
    ...
    const handleDragStart = useCallback(
        (event) => {
            ...
            const isCardDrag = Object.values(cards)
                .flat()
                .some((c) => c._id === activeId);
        ...
        async (event) => {
            ...
            try {
                if (sourceListId !== targetListId) {
                    ...
                    await queuedPut(`/cards/${activeId}`, {
                        list: targetListId,
                        order: newIndex,
                    });
                } else {
                    ...
                    console.log(
                        "🔃 REORDERING WITHIN SAME LIST - sending to server",
                    );
                ...
                console.log(
                    "✅ Server request sent; awaiting cards:reordered broadcast",
                );
            } catch (error) {
    ...
    const handleListDragEnd = useCallback(
        async ({ active, over }) => {
            ...
            const moved = arrayMove(lists, oldIndex, newIndex).map((l, i) => ({
                ...l,
                order: i,
            }));
            console.log(
            ...
            try {
                const payload = {
                    order: moved.map((l, i) => ({ id: l._id, order: i })),
                };
    ...
    const handleCardUpdate = (updatedCard, isDeleted = false) => {
        if (isDeleted) {
            ...
            for (const listId in newCards) {
                newCards[listId] = newCards[listId].filter(
            ...
            const newCards = { ...cards };
            for (const listId in newCards) {
                const cardIndex = newCards[listId].findIndex(
    ...
    const handleSaveTitle = async () => {
        ...
        try {
            const { data } = await api.put(`/boards/${id}`, {
    ...
    useEffect(() => {
        ...
        const onBlur = () =>
            s.emit("typing:boardTitle", { boardId: id, isTyping: false });
        return () => onBlur();
    }, [isEditingTitle, id]);
    ...
    return (
        <>
                                ...
                                {Object.values(cursors).map((c) => (
                                    ...
                                    >
                                        <div className="flex items-center gap-1 translate-x-2 -translate-y-5">
                                            ...
                                            ></span>
                                            <span className="px-1.5 py-0.5 text-[10px] rounded bg-black/70 text-white">
                                                {c.name}
                                            </span>
                                        </div>
                                    </div>
                            ...
                            <div className="flex space-x-3 sm:space-x-4 min-h-[calc(100vh-120px)]">
                                ...
                                <div className="flex-shrink-0 w-64 sm:w-72">
                                    <form
                                        ...
                                        className="bg-white bg-opacity-90 rounded-lg p-2 sm:p-3"
                                    >
                                        <Input
                                            type="text"
                                            value={newListTitle}
                ...
                >
                    <DialogContent>
                        <DialogHeader>
                            ...
                            <DialogDescription>
                                Are you sure you want to move "{board?.title}"
                                to trash? This action cannot be undone.
                            </DialogDescription>
                        </DialogHeader>
                        <DialogFooter>
                            <Button
            ...
            <Dialog
                open={showForceLeaveDialog}
                onOpenChange={setShowForceLeaveDialog}
            >
                <DialogContent>
                    <DialogHeader>
